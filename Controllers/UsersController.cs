    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetInventoriesByUserId()
        {
            try
            {
                ...
                List<ShortInventory> shortInventories = await this.serviceUnitOfWork.UserService.GetShortInventoriesByUserIdAsync(Convert.ToInt32(userId));

                return Ok(new ResponseDto<List<ShortInventory>>()
                {
                    IsSuccess = true,
                    Message = "Successful",
